...
void schedule(int channel)
{
	...
	{//if the code comes here there are instructions to drain.
		...
		/* code starts here */
			...
			if(sts[channel][0] < (NUM_RANKS) && sts[channel][1] < (NUM_BANKS -1)){ //increase bank number safely w/o rank
				sts[channel][1] += 1;
			}
			else{
				if((sts[channel][0] == (NUM_RANKS - 1)) && ((sts[channel][1] == (NUM_BANKS - 1)))){
					sts[channel][0] = 0;
					sts[channel][1] = 0;
				}
				else{
					if((sts[channel][0] < (NUM_RANKS)) && ((sts[channel][1] == (NUM_BANKS -1)))){
						sts[channel][0] += 1;
						sts[channel][1] = 0;
					}
				}
